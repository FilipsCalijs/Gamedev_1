...
class Player(pygame.sprite.Sprite):
    def __init__(self):
	    ...
	    self.health = 5 #колько сердец у персонажа
	    self.max_health = 15
    def get_damage(self):#тут ты просто получаешь урон
        if self.health > 0:
            self.health -= 1
    def get_health(self):
    ...
    def full_hearts(self):#эта система сердец, где отнимаються полное сердце и неостовляет за сабой НИ-ЧЕ-ГО
        for heart in range(self.health):
            screen.blit(full_heart,(heart * 45, 45))#2 и 3 это расположение сердец
    def empty_hearts(self):
        for heart in range(self.max_health):
            if heart < self.health:
                screen.blit(full_heart,(heart * 45, 5))
            else:
                 screen.blit(empty_heart,(heart * 45, 5))
